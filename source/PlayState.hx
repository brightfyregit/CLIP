...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'ray':
				dialogue = CoolUtil.coolTextFile(Paths.txt('ray/dialog'));
				cutdialogue = CoolUtil.coolTextFile(Paths.txt('ray/cutDialog'));
			case 'bonk':
				dialogue = CoolUtil.coolTextFile(Paths.txt('bonk/dialog'));
			case 'limbs':
				dialogue = CoolUtil.coolTextFile(Paths.txt('limbs/dialog'));
				cutdialogue = CoolUtil.coolTextFile(Paths.txt('limbs/cutDialog'));
			case 'nutella':
				dialogue = CoolUtil.coolTextFile(Paths.txt('nutella/dialog'));
			case 'are-you-nuts':
				dialogue = CoolUtil.coolTextFile(Paths.txt('are-you-nuts/dialog'));
		}
		switch(SONG.song.toLowerCase())
		{
			...
			case 'ray' | 'bonk' | 'limbs' | 'paperclips': 
			{
				defaultCamZoom = 0.5;
				curStage = 'town';
	
				var bg:FlxSprite = new FlxSprite(0, -200);
				if (SONG.song.toLowerCase() == 'ray' || SONG.song.toLowerCase() == 'paperclips')
				{
					bg.loadGraphic(Paths.image('city/ocean'));
				}
				else if (SONG.song.toLowerCase() == 'bonk')
				{
					bg.loadGraphic(Paths.image('city/sunset_ocean'));
				}
				else
				{
					bg.loadGraphic(Paths.image('city/sunset_ocean'));
				...
				var citybg:FlxSprite = new FlxSprite(-770, -200);
				if (SONG.song.toLowerCase() == 'ray' || SONG.song.toLowerCase() == 'paperclips')
				{
					citybg.loadGraphic(Paths.image('city/port'));
				}
				else if (SONG.song.toLowerCase() == 'bonk')
				{
					citybg.loadGraphic(Paths.image('city/sunset_port'));
				}
				else
				{
					citybg.loadGraphic(Paths.image('city/port_fire'));
				}
				citybg.setGraphicSize(Std.int(citybg.width * 1.5));
				citybg.antialiasing = true;
				citybg.scrollFactor.set(0.9, 0.9);
				citybg.active = false;
				add(citybg);

				if (SONG.song.toLowerCase() == 'limbs')
				{
					var fya:FlxSprite = new FlxSprite(-320, -270);
					fya.frames = Paths.getSparrowAtlas('city/fire');
					fya.animation.addByPrefix('flames', 'fyaaa', 24, true);
					fya.setGraphicSize(Std.int(citybg.width * 0.6));
					fya.antialiasing = true;
					fya.scrollFactor.set(0.9, 0.9);
					fya.animation.play('flames');
					add(fya);
				}
		
	
				upperBoppers = new FlxSprite(-500, -500);
				upperBoppers.frames = Paths.getSparrowAtlas('city/background_boppers');
				if (SONG.song.toLowerCase() == 'bonk')
				{
					upperBoppers.frames = Paths.getSparrowAtlas('city/background_boppers_bonk');
				}
				upperBoppers.antialiasing = true;
				upperBoppers.scrollFactor.set(0.95, 0.95);
				upperBoppers.setGraphicSize(Std.int(upperBoppers.width * 1.2));
				upperBoppers.updateHitbox();
				if (SONG.song.toLowerCase() == 'limbs')
				{
					upperBoppers.frames = Paths.getSparrowAtlas('city/dagang');
					upperBoppers.animation.addByPrefix('bop', 'limbs_boppers', 24, false);
					upperBoppers.y += 250;
					upperBoppers.x -= 100;
				...
				{
					upperBoppers.animation.addByPrefix('bop', 'background boppers', 24, false);
				}
				add(upperBoppers);
	
				bottomBoppers = new FlxSprite();
				bottomBoppers.frames = Paths.getSparrowAtlas('city/fron');
				if (SONG.song.toLowerCase() == 'bonk')
				{
					bottomBoppers.frames = Paths.getSparrowAtlas('city/fronsun');
				}
				bottomBoppers.animation.addByPrefix('bop', 'a', 24, false);
				bottomBoppers.setGraphicSize(Std.int(bottomBoppers.width * 1.5));
				bottomBoppers.antialiasing = true;
				bottomBoppers.screenCenter();
				bottomBoppers.y += 800;
				bottomBoppers.scrollFactor.set(0.97, 0.97);
			...
			case 'nutella' | 'are-you-nuts': 
			{
				curStage = 'forest';
				defaultCamZoom = 0.75;
				
				var bg:FlxSprite = new FlxSprite(500, 290);
				if (SONG.song.toLowerCase() == 'nutella' || SONG.song.toLowerCase() == 'are-you-nuts')
				bg.loadGraphic(Paths.image('forest/forestBG'));
				...
				var forestbg:FlxSprite = new FlxSprite(-400, -700);
				if (SONG.song.toLowerCase() == 'nutella' || SONG.song.toLowerCase() == 'are-you-nuts')
				forestbg.loadGraphic(Paths.image('forest/trees'));
				forestbg.setGraphicSize(Std.int(forestbg.width * 1.5));
				forestbg.antialiasing = true;
				forestbg.active = false;
				add(forestbg);
			}
			default:
			{
		...
		if (curStage == 'town')
			gfVersion = 'hazel-speaker';

		gf = new Character(400, 130, gfVersion + addons);
		gf.scrollFactor.set(0.95, 0.95);

		dad = new Character(100, 100, SONG.player2 + addons);
		...
		switch (SONG.song.toLowerCase())
		{
			case "ray":
				if (isStoryMode)
				{
					camPos.set(dad.getGraphicMidpoint().x - 400, dad.getGraphicMidpoint().y - 200);
				}
		}

		
		boyfriend = new Boyfriend(770, 450, SONG.player1 + addons);
		...
		gfWarnSpr = new FlxSprite(gf.x, gf.y - 45);
		gfWarnSpr.frames = Paths.getSparrowAtlas('combat/prepare', 'shared');
		switch (SONG.song.toLowerCase())
		{
			case 'bonk':
				gfWarnSpr.frames = Paths.getSparrowAtlas('combat/prepare_bonk', 'shared');
			case 'limbs':
				gfWarnSpr.frames = Paths.getSparrowAtlas('combat/prepare_limbs', 'shared');
		}
		gfWarnSpr.animation.addByPrefix('warn', 'GF attacking beat', 24, false);
		gfWarnSpr.antialiasing = true;
		gfWarnSpr.alpha = 0;
		gfWarnSpr.scrollFactor.set(0.95, 0.95);
		add(gfWarnSpr);
		...
		combatSpr = new FlxSprite();
		combatSpr.frames = Paths.getSparrowAtlas('combat/COMBAT', 'shared');
		combatSpr.animation.addByPrefix('attack', 'ATK', 24, false);
		combatSpr.animation.addByPrefix('dodge', 'DGE', 24, false);
		combatSpr.antialiasing = true;
		combatSpr.alpha = 0;
		combatSpr.setGraphicSize(Std.int(combatSpr.width * 2));
		combatSpr.screenCenter();
		combatSpr.scrollFactor.set();
		combatSpr.y -= 200;
		combatSpr.x += 200;
		add(combatSpr);
		...
		var cutdoof:DialogueBox = new DialogueBox(false, cutdialogue);
		cutdoof.scrollFactor.set();
		cutdoof.finishThing = cutRunSchoolIntro;
		cutdoof.cut = true;
	...
	function cutscene(?dialogueBox:DialogueBox):Void
	{
	...
	function cutRunSchoolIntro():Void
	{
		healthBarBG.visible = false;
		kadeEngineWatermark.visible = false;
		healthBar.visible = false;
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		var cliplanding:FlxSprite = new FlxSprite();
		cliplanding.frames = Paths.getSparrowAtlas('city/superherodrop');
		cliplanding.animation.addByPrefix('idle', 'everythin', 24, false);
		cliplanding.setGraphicSize(Std.int(senpaiEvil.width * 1));
		cliplanding.scrollFactor.set();
		cliplanding.antialiasing = true;
		cliplanding.updateHitbox();
		cliplanding.screenCenter();
		cliplanding.x -= 300;
		cliplanding.y += -40;
		...
		if (SONG.song.toLowerCase() == 'ray')
		{
			upperBoppers.alpha = 0;
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer)
		{
			...
			else
			{
				if (dialogueBox != null)
				{
					...
					else if (SONG.song.toLowerCase() == 'ray')
					{
						FlxG.sound.play(Paths.sound('CutsceneAmbience'));
						new FlxTimer().start(1, function(swagTimer:FlxTimer)
						{
							...
							{
								add(cliplanding);
								cliplanding.animation.play('idle', true, false, 30);
	...
	override public function update(elapsed:Float)
	{
		...
		if (dad.animation.curAnim.name == 'attack' || dad.animation.curAnim.name == 'hurt')
		{
		...
		if (boyfriend.animation.curAnim.name == 'dodge' || boyfriend.animation.curAnim.name == 'attack' || boyfriend.animation.curAnim.name == 'hurt' || boyfriend.animation.curAnim.name == 'pre-attack')
		{
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (SONG.song.toLowerCase() == "limbs")
						FlxG.switchState(new EndingState(misses <= 50));
					else
	...
	private function keyShit():Void
	{
		...
		if ((upP || rightP || downP || leftP) && !boyfriend.stunned && generatedMusic)
			...
			{
				if (boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.animation.curAnim.name.endsWith('miss') && boyfriend.animation.curAnim.name != 'dodge' && boyfriend.animation.curAnim.name != 'attack' && boyfriend.animation.curAnim.name != 'hurt' && boyfriend.animation.curAnim.name != 'pre-attack')
				{
					boyfriend.playAnim('idle');
				}
			}
	...
	*/
	...
	function bfHurt()
	{
		new FlxTimer().start(0.2, function(tmr:FlxTimer)
		{
			...
			health -= 0.75;
			songScore -= 75;
			FlxG.sound.play(Paths.sound('combat/bfhurt'));
			FlxG.sound.play(Paths.sound('combat/bonc'));
	...
	function bfAttack()
	{
		...
		FlxG.sound.play(Paths.sound('combat/micthrow'));
	...
	function dadHurt()
	{
		...
		{
			FlxG.sound.play(Paths.sound('combat/cliphurt'));
	...
	function dadAttack()
	{
		...
		FlxG.sound.play(Paths.sound('combat/batswing'));
	}
	...
	{
		trace('bf attack started');
		gfWarn();
		combatSpr.animation.play('attack', true);
		pressedSpace = false;
		detectAttack = true;
	...
	function dodgeEvent()
	{
		trace('dad attack started');
		gfWarn();
		combatSpr.animation.play('dodge', true);
		pressedSpace = false;
		detectAttack = true;
		new FlxTimer().start(1, function(tmr:FlxTimer)
		{
	...
	function detectInput()
	{
		if (FlxG.keys.justPressed.SPACE)
		{
			...
			FlxG.sound.play(Paths.sound('combat/spacepressedcharge'));
		}
		if (timesPressed >= 3)
		{
			trace('clicked all of the times');
			pressedSpace = true;
			detectAttack = false;
			//FlxG.camera.flash(FlxColor.WHITE, 0.2);
			FlxG.sound.play(Paths.sound('combat/spacepressed'));
	...
	function gfWarn()
	{
		timesPressed = 0;
		combatSpr.alpha = 1;
		FlxG.sound.play(Paths.sound('combat/attackwarn'));
		FlxFlicker.flicker(combatSpr, 1.5, 0.1, false);
		gf.alpha = 0;
		gfWarnSpr.alpha = 1;
		gfWarnSpr.animation.play('warn', true);
	...
	override function stepHit()
	{
		...
		else if (SONG.song.toLowerCase() == 'bonk')
		{
			switch curStep
			{
				case 672 | 736 | 1183 | 1247:
					dodgeEvent();
				case 639 | 704 | 1150 | 1214:
					attackEvent();
			}
		}
		else if (SONG.song.toLowerCase() == 'limbs')
		{
			switch curStep
			{
				case 16:
	...
	override function beatHit()
	{
		...
		if (SONG.notes[Math.floor(curStep / 16)] != null)
		{
			...
			if (SONG.notes[Math.floor(curStep / 16)].mustHitSection && dad.animation.curAnim.name != 'attack' && dad.animation.curAnim.name != 'hurt')
				dad.dance();
		}
		...
		if (!boyfriend.animation.curAnim.name.startsWith("sing") && boyfriend.animation.curAnim.name != 'dodge' && boyfriend.animation.curAnim.name != 'attack' && boyfriend.animation.curAnim.name != 'hurt' && boyfriend.animation.curAnim.name != 'pre-attack')
		{
			boyfriend.playAnim('idle');
		}
